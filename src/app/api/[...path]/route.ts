...
async function chatData(c:Conversation,owner:string){const [chatMessages,ownerMemories,relationships,loadedCharacters,world,persona]=await Promise.all([messages(c.id),memories(owner),db().prepare('SELECT character_id,trust,note FROM relationships WHERE owner_id=? AND scope=?').all(owner,scope(c)),Promise.all(c.character_ids.map(cid=>get<Character>('characters',cid,owner))),c.world_id?get<World>('worlds',c.world_id,owner):Promise.resolve(null),c.persona_id?get<Persona>('personas',c.persona_id,owner):Promise.resolve(null)]);return {conversation:c,messages:chatMessages,memories:ownerMemories.filter(m=>c.world_id?m.world_id===c.world_id&&m.persona_id===c.persona_id:!m.world_id&&m.character_id===c.character_ids[0]),relationships,characters:loadedCharacters.filter((character):character is Character=>!!character),world,persona};}
async function handle(req:NextRequest,ctx:Context){
 ...
 if(path[0]==='auth'){
  ...
  if(path[1]==='google'&&method==='GET'){
   ...
   c.set('sora_oauth',JSON.stringify({state,verifier}),{httpOnly:true,secure:process.env.NODE_ENV==='production'||process.env.APP_URL?.startsWith('https://')===true,sameSite:'lax',path:'/api/auth',maxAge:600});
   const redirectUri=`${applicationUrl()}/api/auth/callback`;const url=new URL('https://accounts.google.com/o/oauth2/v2/auth');url.search=new URLSearchParams({client_id:process.env.GOOGLE_CLIENT_ID!,redirect_uri:redirectUri,response_type:'code',scope:'openid profile email',state,code_challenge:createHash('sha256').update(verifier).digest('base64url'),code_challenge_method:'S256'}).toString();return NextResponse.redirect(url);
  }
  if(path[1]==='callback'&&method==='GET'){
   const c=await cookies(),raw=c.get('sora_oauth')?.value;c.delete('sora_oauth');const saved=raw?parseOAuthState(raw):null;if(!saved)fail('Sign-in expired. Please try again.',401);if(!matchesOAuthState(req.nextUrl.searchParams.get('state'),saved.state))fail('Sign-in state did not match. Please try again.',401);
   ...
   if(!googleReady())fail('Google sign-in is not configured.',503);const token=await fetch('https://oauth2.googleapis.com/token',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:new URLSearchParams({code,client_id:process.env.GOOGLE_CLIENT_ID!,client_secret:process.env.GOOGLE_CLIENT_SECRET!,redirect_uri:`${applicationUrl()}/api/auth/callback`,grant_type:'authorization_code',code_verifier:saved.verifier}),signal:AbortSignal.timeout(20000)});if(!token.ok)fail('Google sign-in failed. Please try again.',401);
   const tokenPayload=z.object({access_token:z.string().min(1)}).safeParse(await token.json());if(!tokenPayload.success)fail('Google sign-in failed. Please try again.',401);const info=await fetch('https://openidconnect.googleapis.com/v1/userinfo',{headers:{Authorization:`Bearer ${tokenPayload.data.access_token}`},signal:AbortSignal.timeout(20000)});if(!info.ok)fail('Unable to read Google profile.',401);const parsedProfile=googleProfileSchema.safeParse(await info.json());if(!parsedProfile.success)fail('A verified Google account is required.',401);const profile=parsedProfile.data;
   ...
   if(!uid){uid=user?.guest?user.id:id();if(user?.guest)(await db().prepare('UPDATE users SET google_sub=?,name=?,picture=?,guest=0 WHERE id=?').run(profile.sub,profile.name||'Traveler',profile.picture||null,uid));else (await db().prepare('INSERT INTO users (id,google_sub,name,picture,guest) VALUES (?,?,?,?,0)').run(uid,profile.sub,profile.name||'Traveler',profile.picture||null));}
   await session(uid,c.get('sora_session')?.value);return NextResponse.redirect(new URL('/',process.env.APP_URL||req.url));
  }
 }
 ...
 if(path[0]==='worlds'&&method==='POST'){
  ...
  if(path[2]==='characters'){const {character_id}=z.object({character_id:z.string()}).parse(await body(req));if(!(await get<Character>('characters',character_id,owner)))fail('Character not found.',404);(await db().prepare('INSERT OR IGNORE INTO world_characters (world_id,character_id) VALUES (?,?)').run(world.id,character_id));return json({ok:true});}
 }
 ...
 if(path[0]==='conversations'){
  if(method==='POST'&&path.length===1)return json((await startConversation(owner,conversationSchema.parse(await body(req)))),201);
 ...
 if(path[0]==='avatars'&&method==='POST'){const input=z.object({asset_url:z.string().url().max(2000).refine(s=>s.startsWith('https://')&&/\.vrm(?:\?|$)/i.test(s),'Use an HTTPS URL to a .vrm file. Allow cross-origin access on its host.'),type:z.literal('vrm')}).parse(await body(req));const aid=id();(await db().prepare('INSERT INTO avatars (id,owner_id,type,asset_url) VALUES (?,?,?,?)').run(aid,owner,input.type,input.asset_url));return json({id:aid},201);}
...
async function route(req:NextRequest,ctx:Context){try{return await handle(req,ctx);}catch(e){if(e instanceof z.ZodError)return json({error:e.issues.map(i=>i.message).join(' ')},400);if(e instanceof AppError||e instanceof DatabaseContextError)return json({error:e.message},e.status);console.error('Request failed:',e instanceof Error?e.message:'Unknown error');return json({error:'Something went wrong. Please try again.'},500);}}
