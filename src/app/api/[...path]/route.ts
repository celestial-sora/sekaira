...
async function handle(req:NextRequest,ctx:Context){
 ...
 if(path[0]==='auth'){
  ...
  if(path[1]==='callback'&&method==='GET'){
   ...
   if(!uid){uid=user?.guest?user.id:id();if(user?.guest)(await db().prepare('UPDATE users SET google_sub=?,name=?,guest=0 WHERE id=?').run(profile.sub,profile.name||'Traveler',uid));else (await db().prepare('INSERT INTO users (id,google_sub,name,guest) VALUES (?,?,?,0)').run(uid,profile.sub,profile.name||'Traveler'));}
