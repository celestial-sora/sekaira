...
export async function POST(req: NextRequest, ctx: Context) {
  ...
  const changed = await setPublished("characters", id, user.id, input.data.published);
  if (!changed) return json({ error: "Character not found or not owned by this account." }, 404);
