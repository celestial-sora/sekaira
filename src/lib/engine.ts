...
export async function runTurn(conv:Conversation,chars:Character[],world:World|null,persona:Persona|null,content:string,selected?:string[]){
 ...
 (await transaction(async ()=>{
  ...
  for(const {character:c,reply:r} of replies){(await addMessage(conv.id,'assistant',c.id,r.dialogue,r.emotion));(await db().prepare('INSERT INTO relationships (owner_id,scope,character_id,trust,note) VALUES (?,?,?,?,?) ON CONFLICT(owner_id,scope,character_id) DO UPDATE SET trust=MAX(-100,MIN(100,trust+excluded.trust)),note=CASE WHEN excluded.note=\'\' THEN note ELSE excluded.note END').run(conv.owner_id,scope(conv),c.id,r.trust_delta,r.relationship_note));}
