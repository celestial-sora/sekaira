...
export async function guest(){const existing=await currentUser();if(existing)return existing;if(!guestAllowed())throw new Error('Sign in with Google to continue.');const user={id:id(),name:'Traveler',guest:true};(await db().prepare('INSERT INTO users (id,google_sub,name,guest) VALUES (?,?,?,?)').run(user.id,null,user.name,1));await session(user.id);return user;}
