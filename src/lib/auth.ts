...
export const configuredAdmin=(email:string|null|undefined)=>{const configured=process.env.ADMIN_EMAILS?.split(',').map(v=>v.trim().toLowerCase()).filter(Boolean)??[];return Boolean(email&&configured.includes(email.toLowerCase()));};
const adminAccess=(email:string|null|undefined,persisted=false)=>process.env.NODE_ENV==='production'?configuredAdmin(email):persisted||configuredAdmin(email);
export async function currentUser(){const token=(await cookies()).get('sora_session')?.value;if(!token)return null;const r=(await db().prepare('SELECT u.id,u.email,u.name,u.picture,u.guest,u.admin,u.age_range,u.age_verified FROM users u JOIN sessions s ON s.user_id=u.id WHERE s.token_hash=? AND s.expires_at>?').get(hash(token),Date.now()));if(!r)return null;const age_range:'general'|'mature'=r.age_range==='mature'?'mature':'general';const email=(r.email as string|null)||null;return {id:r.id as string,email,name:r.name as string,picture:(r.picture as string|null)||null,guest:!!r.guest,admin:adminAccess(email,!!r.admin),age_range,age_verified:r.age_verified === undefined ? true : !!r.age_verified};}
export const isAdmin=(user:{admin?:boolean;email?:string|null}|null)=>Boolean(user&&adminAccess(user.email,!!user.admin));
