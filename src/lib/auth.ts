...
export async function currentUser(){const token=(await cookies()).get('sora_session')?.value;if(!token)return null;const r=(await db().prepare('SELECT u.id,u.email,u.name,u.picture,u.guest,u.admin,u.age_range,u.age_verified FROM users u JOIN sessions s ON s.user_id=u.id WHERE s.token_hash=? AND s.expires_at>?').get(hash(token),Date.now()));return r?{id:r.id as string,email:(r.email as string|null)||null,name:r.name as string,picture:(r.picture as string|null)||null,guest:!!r.guest,admin:!!r.admin,age_range:(r.age_range as string)||'general',age_verified:r.age_verified === undefined ? true : !!r.age_verified}:null;}
export const isAdmin=(user:{admin?:boolean;email?:string|null}|null)=>{const configured=process.env.ADMIN_EMAILS?.split(',').map(v=>v.trim().toLowerCase()).filter(Boolean)||[];return Boolean(user?.admin || (user?.email && configured.includes(user.email.toLowerCase())));};
