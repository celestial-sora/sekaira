...
export const googleProfileSchema=z.object({sub:z.string().min(1).max(255),name:z.string().max(200).optional(),email_verified:z.literal(true)}).passthrough();
...
export async function session(userId:string,previousToken?:string){const token=randomBytes(32).toString('base64url');await transaction(async()=>{await db().prepare('DELETE FROM sessions WHERE expires_at<?').run(Date.now());if(previousToken)await db().prepare('DELETE FROM sessions WHERE token_hash=?').run(hash(previousToken));await db().prepare('INSERT INTO sessions VALUES (?,?,?)').run(hash(token),userId,Date.now()+30*86400000);});(await cookies()).set('sora_session',token,{httpOnly:true,secure:secureCookies(),sameSite:'lax',path:'/',maxAge:30*86400});}
