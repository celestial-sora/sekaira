...
export const isAdmin=(user:{admin?:boolean;email?:string|null}|null)=>Boolean(user?.admin || (user?.email && (process.env.ADMIN_EMAILS||'suphloeksangko@gmail.com').split(',').map(v=>v.trim().toLowerCase()).includes(user.email.toLowerCase())));
