...
export function postgresSQL(sql:string){
 let out=sql.replace(/\bIS \?/g,'IS NOT DISTINCT FROM ?').replace('MAX(-100,MIN(100,trust+excluded.trust))','GREATEST((-100)::bigint,LEAST((100)::bigint,relationships.trust+excluded.trust))');
